...
public abstract class Character {
    ...
    public boolean move(Pair<Tile, int[]> currentTile, Pair<Tile, int[]> targetTile) {
        ...
        // Appel à AStar pour trouver le chemin
        List<Node> path = AStarMoveStrategy.findPath(currentTile, targetTile, arrayTiles, charInClass);
    ...
    private boolean moveAlongPath(List<Node> path, Pair<Tile, int[]> currentPosition) {
        ...
        for (Node node : path) {
            ...
            if (!tile.isObstacle() && charInClass.get(node.getX()).get(node.getY()) == null) {
                ...
                classroom.charPosChanged(charInClass.get(newX).get(newY), oldX, oldY, newX, newY);
    ...
    protected static void updatePosition(Character character, Tile tile, int[] position) {

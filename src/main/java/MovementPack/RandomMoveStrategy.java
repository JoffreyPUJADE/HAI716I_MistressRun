...
public class RandomMoveStrategy extends Character {
    ...
    private static List<Node> getNeighbors(Node node, ArrayList<ArrayList<Tile>> tiles,
        ...
        int[] dx = {-1, 1, 0, 0};  // Déplacement horizontal
        int[] dy = {0, 0, -1, 1};  // Déplacement vertical
        ...
        if (newX < 0 || newX >= tiles.size() || newY < 0 || newY >= tiles.get(0).size()) {
        continue;
        }

        // Vérification si la case est occupée par un obstacle dynamique
        boolean isBlocked = false;
        for (Character character : characters) {
        if (character.getX() == newX && character.getY() == newY) {
        isBlocked = true;  // La case est occupée par un personnage ou un ennemi
        break;
        }
        }

        if (isBlocked) {
        continue;  // Ignorer cette case si elle est bloquée
        }
    ...
    public RandomMoveStrategy(String spriteSheet, Chair chair, int i, int j) {
